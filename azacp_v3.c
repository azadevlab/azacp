...
void* aza_copy(void *arg){
	PART *file_part = (PART *)arg;
	int file_input, file_out, x, i = 0;
	char data[SIZE];

	char local_file_src[79];
	char local_files_dst[79];
	strncpy(local_file_src, file_src, sizeof(local_file_src) - 1);
	local_file_src[sizeof(local_file_src) - 1] = '\0';
	strncpy(local_files_dst, files_dst, sizeof(local_files_dst) - 1);
	local_files_dst[sizeof(local_files_dst) - 1] = '\0';

	file_input = open(local_file_src, O_RDONLY);
    file_out = open(local_files_dst, O_WRONLY);
